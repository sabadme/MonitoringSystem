...
$sqlNotif = mysql_query("SELECT * FROM admin_notif WHERE popUpValue='1' ORDER BY id desc");
while($data_sqlNotif = mysql_fetch_array($sqlNotif)){
	  $reportIDCheck = $data_sqlNotif['report_id'];
	  	...
	  	if($reportIDCheck == "None"){
	  		...
	  		?>
	  		<div class="alert alert_default"><span><?php echo $booker; ?>
	  		<?php 
	  		...
	  		$prioroty_status = $dataEquipment['priority_status'];
	  		  ...
	  		  ?>
	  		  <div class="alert alert_default"><span><?php echo $userName; ?>
	  		   <?php 
