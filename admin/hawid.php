...
if(isset($_REQUEST['equipment'])){
	if(isset($_REQUEST['groupQuantity'])){
		 $inputGroupQuantity = $_REQUEST['groupQuantity'];
		 $equipment=$_REQUEST['equipment'];

	 for ($i=0; $i < count($equipment) And count($inputGroupQuantity) ; $i++) { 
	 		 	echo $equip=$equipment[$i];
	 		 	echo $equipmentGroup[$i];

	 		 $sql_EquipmentCheck = mysql_query("SELECT * FROM equipment WHERE id='$equip' OR equipment_name='$equip'");
	 		 $data_EquipmentCheck = mysql_fetch_array($sql_EquipmentCheck);
	 		 $equipmentCheckName = $data_EquipmentCheck['equipment_name'];
	 		 $equipmentCheckId = $data_EquipmentCheck['id'];

	 		 if($equipmentCheckName == $equip){

	 		 	$sql_lastCheck = mysql_query("SELECT * FROM equipment WHERE id='$equipmentCheckName'");
	 		 	$data_lastCheck = mysql_fetch_array($sql_lastCheck);
	 		 	$finalID = $data_lastCheck['id'];

	 		 	for ($i=0; $i < $equipmentGroup ; $i++) { 
	 		 ...
	 		 }else if($equipmentCheckId == $equip){
	 		 		$insert=mysql_query("INSERT INTO booking VALUES(0,'$booker','$venue','$sem','$datestart','$dateend','$timestart','$tameend','$equip','Pending','0','1')");
						echo mysql_error();  
					  if($insert){
